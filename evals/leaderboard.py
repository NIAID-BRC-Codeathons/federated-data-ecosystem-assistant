...
HAND = {("gpt41nano", "B15"): ("miss_fig", "GEO Series",
        ...
        ("gpto3", "B13"): ("miss_fig", "633 254",
                           "adds GEO studies to SRA runs: 1,929 / 633,254 'combined records'")}
...
for d in sorted(RUNS.glob("*-bobby-lanes")):
    ...
    for f in sorted(d.glob("b*.jsonl")):
        ...
        # ---- per-model scores (B8 is not scored at all)
        if o == "not_asked":
            r["not_asked"] += 1
            continue
        if qid == "B8":
...
reruns = [m for m in models if "-" in m["model"]]          # direct-API reruns of Argo models
n_distinct = len(models) - len(reruns)
runs_meta = Counter()
for d in RUNS.glob("*-bobby-lanes"):
    for f in d.glob("b*.jsonl"):
        s = summary(load(f))
        if s:
            runs_meta[(s.get("run_id") or "").split("-")[0] + " " + (s.get("code_sha") or "")] += 1
run_when, run_n = runs_meta.most_common(1)[0]
other_srv = {q: sum(1 for m in cells.values() if q in m and m[q]["o"] == "routed"
                    and not any(t.startswith(OURS) for t in m[q]["tools"])) for q in ANY_TOOL}
calls_hist = Counter(min(len(c["tools"]), 2) for m in cells.values() for q, c in m.items()
                     if c["o"] not in ("not_asked", "defect"))
...
w("*Generated by `uv run evals/leaderboard.py` from the transcripts in `evals/runs/*-bobby-lanes/`. "
  "Do not edit by hand.*\n")
w(f"16 plain-language questions were asked of {n_distinct} models in {len(models)} runs (three "
  f"Claude models also ran directly on Anthropic's API, and those runs stopped early), through the "
  f"agent setup `chatbot.py` uses, with every team server's tools loaded. {len(answered_all)} runs answered all 16; "
  ...
  "two servers.\n")
w("## The 16 questions\n")
w("Six were written for GEO, six for BRC/ENA, and four for both. Two of those (B4, B16) name no "
  "database, so any tool counts there and the table marks them \"any\"; "
  f"{other_srv['B16']} right B16 answers and {other_srv['B4']} right B4 answers used only other "
  "team servers. Each is a question a researcher might type, written so that the obvious call "
  "gets it wrong in one specific way.\n")
...
w("Never ran: " + ", ".join(m["model"] for m in never) + ". Argo's Gemini route rejected one tool "
  "definition in the combined list (an integer enum where Gemini needs strings); it is not in "
  "geo.py or brc_analytics.py and has not been traced.\n")
...
w(f"- About half the scored answers ({calls_hist[0] + calls_hist[1]} of {sum(calls_hist.values())}) "
  "took one tool call or none. The hard part was usually choosing the right tool and reading its "
  "result, not chaining calls.")
w(f"- All scored runs used code from {run_when[:4]}-{run_when[4:6]}-{run_when[6:8]} ({run_when.split()[1]}), "
  "before the team's system-prompt refactor, the provenance change and the last two fixes to "
  "these servers. The results describe that code.")
w("- The figure check looks for the number, not for what the answer calls it. Two answers were "
  "failed by hand for mixing units: gpt41nano B15 calls 181,408 \"GEO Series\", and o3 B13 "
  "divides by GEO studies plus SRA runs. All 49 B13/B15 answers scored right were scanned for "
  "the same mistake.")
...
w(f"- Prices were read on {PRICES['read_on']} from the vendors' pages ({', '.join(PRICES['sources'].values())}). "
  "Argo IDs were matched to models by spelling. No prompt caching is assumed. Argo itself charges "
  "nothing. The `list_cost_usd` field inside the transcripts is an older estimate from memory "
